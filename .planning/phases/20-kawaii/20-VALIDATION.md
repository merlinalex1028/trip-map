...
|----------|-------------|------------|-------------------|
| Hover / active motion feels airy rather than jittery on real popup placement | INTER-01, INTER-02, INTER-03 | Automated tests can assert classes, but not real pointer feel against anchored popup positioning | Start web app, open a popup from the map, hover primary CTA, secondary CTA, and inner cloud card; confirm lift is subtle and popup anchor does not drift |
| Topbar sweetness increases without reclaiming first-screen map height | STYLE-05 | Visual density and first-screen balance depend on actual viewport rendering | Compare homepage before/after on desktop width, confirm header stays at `h-14 md:h-16` and map remains the main first-screen surface |
