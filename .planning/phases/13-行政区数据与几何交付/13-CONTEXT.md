...
Phase 13 负责把 v3.0 所需的中国市级与海外一级行政区几何数据来源、版本清单、ID 映射、静态资产交付方式，以及坐标适配验证规则固定下来。它的交付目标是让后续 `Leaflet` 主链路可以按需获取稳定可追踪的几何资源，而不是在这一阶段完成完整的 `Leaflet` 双图层消费接线、记录 CRUD，或复杂的服务端几何服务化。
...
- `.planning/REQUIREMENTS.md` — `GEOX-03`、`GEOX-04`、`GEOX-06`、`GEOX-07` 与 `API-03` 定义了中国/海外正式数据源、统一 ID / 版本清单，以及几何资源入口约束；`GEOX-05` 的 `Leaflet` 双图层消费接线在 Phase 14 承接。
