# Requirements Archive: v5.0 账号体系与云同步基础版

**Archived:** 2026-04-17
**Status:** SHIPPED

Use `/gsd-new-milestone` to create the next active `.planning/REQUIREMENTS.md`.
...
- `AUTH-01..05`：已验证，邮箱密码账号、sid 会话恢复与账号身份 UI 入口全部闭环。
- `SYNC-01..05`：已验证，current-user ownership、same-user foreground refresh、点亮/取消点亮与提示分流全部闭环。
- `MIGR-01..04`：已验证，首登本地导入、cloud-wins 与 logout / switch-account / unauthorized 边界全部闭环。
- `OVRS-01..03`：已验证，8 国 overseas admin1 支持、persisted metadata replay 与 unsupported popup feedback 全部闭环。
...
*Last updated: 2026-04-17 — archived after v5.0 completion*
